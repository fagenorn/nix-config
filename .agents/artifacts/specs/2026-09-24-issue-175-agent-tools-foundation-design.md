...
4. **Controls.** These prove that each channel is live, so assertion 3 can
   ...
   line, runs `-m agent_tools.<module> --help` three times, each run opening
   exactly one channel, and each run must exit 97 with the marker:
   - (a) without `-I`, with only `PYTHONPATH` set and a neutral working
     directory. This proves the `PYTHONPATH` channel is live on its own.
   - (a′) without `-I`, with neither variable set and the fake's root as the
     working directory. This proves the working-directory channel is live.
...
| D8 | The installed test lives in `tests/` and runs only from `agent-installed-skill-tests`. It enumerates launchers by template match (anything naming `agent_tools` must be one) with an `agent-evidence` floor, then checks prog/name, a hostile `PYTHONPATH`+`NIX_PYTHONPATH`+`.pth`+cwd run, and three single-channel controls (non-isolated with only `PYTHONPATH`; non-isolated with only the cwd; `-I` with only `NIX_PYTHONPATH`, per D14) | Parent D9 seam 2, D13; #153 D8/D16 env handling; the-bar Tests that can fail | Emit the table as a JSON file into the home — a deployed artifact only a test reads. Pin the whole set — a second table |
...
| D14 | Refines D8: the installed test's controls each open exactly one channel — `PYTHONPATH` alone, the working directory alone, `NIX_PYTHONPATH` alone under `-I` — so a dead channel cannot hide behind a live one. Refines D4: `lib/agent-tools.nix` asserts at evaluation that each command-table row's module is in the import-checked set | Phase-5 plan review (Claude fallback) SF-1 — the combined control passed with `PYTHONPATH` dead, and AC1 names `PYTHONPATH` — and D-1 — a misspelt row built a launcher that fails only at run time; the-bar Tests that can fail, Fail loud | One combined non-isolated control — any live channel masks a dead `PYTHONPATH`; leaving the row unchecked — the failure surfaces only in the installed test |
