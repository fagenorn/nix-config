...
| D8 | Make the migration inventory exhaustive across phase-entry skills, retained-snapshot support documents, `context-map-lint`, and release contracts; select the context map only as the sole `CONTEXT-MAP.md` basename in the authored `bindings.paths.context` list, with zero meaning no invocation and multiple meaning stop; assert each configured-review owner/support pair independently; scan only existing tracked living text while asserting deleted legacy paths absent; and make the narrow activation-command patch the first mutation immediately before the sole retained resolution. | Phase-5 `fresh-sol-high` review of `c4b4a0d7b9de4feaf8d48d2497f0c1f50f0ae03e` and residual re-review of `e1003b0` verified that omitted support/helper consumers, ambiguous map selection, concatenated review assertions, unrestricted/deleted-path UTF-8 scans, and a pre-patch test step left the cutover or bootstrap boundary incomplete. | Infer a default map path, accept ambiguous maps, rely on a top-level skill list, let one review document mask another, recursively decode arbitrary or deleted filesystem content, or mutate a test before the one authorized pre-resolution patch. |
| D9 | User ruling (2026-09-24): after syncing with `origin/main`, this delivery's cumulative review range starts at the post-sync merge-base with `origin/main` (`5702dfb3bc758ae16b9cf1520749b2e00a1dc215`), not the original `DELIVERY_BASE` `6d4b7a49dd3a44c079c8310e902a86665a6805f0`; the seven sync conflicts were each resolved by the user's own choice. | Explicit user approval given after run `direct-100-000002` stopped because the fixed range `6d4b7a4..a7b7c6f` was `decompose_required` (member_count, aggregate_bytes) once integrated `main` commits entered it; sdd's pin-once rule otherwise forbids repinning. | Keep the fixed `6d4b7a4` base, which counts integrated `main` commits as this delivery and cannot pass the budget, or split into source-cutover and host-activation deliveries under a derived-delivery rule that does not yet exist (issue 152). |
| D10 | User ruling (2026-09-24): `workflow-state build-delivery`'s internal `resolve-project resolve` is the one sanctioned helper-internal resolution, a sealed read-only derivation owned by main's delivery lifecycle. Every other consumer still resolves once at its phase entry and never again, and the living instructions that forbid another resolver name this exception. | #171's accepted D27 keeps that resolution deliberately: the build module does no I/O, and a resolver refusal is a builder refusal. The user chose this exception over widening this issue's scope. The refusal-detail and resolution-root gaps are tracked in https://github.com/fagenorn/nix-config/issues/181. | Change `build-delivery` to consume the caller's retained snapshot, which rewrites main's lifecycle core beyond this issue's scope, or ship the contradiction with D1 as a known residual. |
| D11 | User ruling (2026-09-24), amending D4's and D6's pre-merge order: installed-mode verification (Task 6) does not run before merge. This issue ships on source verification, `just build`, the permission-guard suite and CI. After the merge, the user runs `just switch` from `main` at a quiet moment, and installed-mode verification runs then: ordinary `just agent-workflow-tests` without the source-mode override, `just agent-installed-skill-tests`, and the installed zero-reference scans. | Activation needs the user at the machine (`sudo` requires a password). It is host-wide, so activating before merge would install an unmerged generation under every running agent session. The previous activation attempt failed at the Homebrew prefix step. | Activate the reviewed branch generation before merge, as D4/D6 originally ordered, or have the agent run `just switch` itself. |
| D12 | Ship-time sync decision (2026-09-24): adopt-project, merged to `main` during this delivery by https://github.com/fagenorn/nix-config/issues/148, names the legacy store `.claude/skills.config.json` and legacy keys (`specDir`, `planDir`, `agentBudgetMinutes`, `maxParallel`) as migration inputs that it reads and rewrites. The living-source scan exempts exactly those tokens in the four named adopt-project files through `LEGACY_MIGRATION_INPUTS`, and fails when an entry stops matching. The `# policy-gate-pattern` line marker is honored only in the two gate test files. | D2 retires legacy policy *consumers*. A migrator must name what it migrates, and deleting those names would break adopt-project. Phase-5 conformance review of `85e7f27` found that the first sweep's per-line markers were an open-ended exemption that any file could claim. | Mark each adopt-project line with the marker, which exempts any token in any file. Strip adopt-project's legacy recognition, which breaks its migration. Exempt whole files, which would hide a future legacy consumer in them. |
