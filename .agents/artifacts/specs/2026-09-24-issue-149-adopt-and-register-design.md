...
| D12 | A1 needs no generation of its own. D1's ask stays a `just switch` on `main`, and the notice names B1's merge-base as the minimum revision (refines D1, no reversal). B2 still compares only the eleven runtime files. The deployed `HUMAN-GATE.md` keeps its `.out-of-scope` link until the first switch after merge. | `test_installed_policy_surface_matches_source_contract` asserts tokens in the installed skills (the skill set, the resolution and refusal sentences, `bindings.*` fields, forbidden legacy names), never bytes, and A1 changes none of them. The agent-skills README is not installed, since `default.nix` publishes only `skills/<dir>`. The installed-view dispatch suites skip without `AGENT_SKILLS_INSTALLED_HOME`. | A switch from the branch head, which deploys unmerged code. Adding A1's files to B2's list, which fails the gate on every head that carries A1, so no operator switch on `main` could ever pass it. |
| D13 | Phase-5 hardening of the owner routes (extends D5 and D10). Any return to the loop at or after B5 first resets to `BASE`, locally and before push. A conflict in a B7 or ship sync merge counts as stale. A change since `M` to the engine sources that build the eleven gate files counts as stale before push, and stops before merge after it. B1 re-runs A1's legacy-reference grep on the synced head. A `plan_id` change that only the worktree count explains (HEAD unchanged, tree clean) re-plans at most three times, with no rebuild. | Plan review (Claude fallback): D3 forbids authored commits after the adoption; a hand-resolved conflict is a hand edit of tool output; `adopt_inspection.py` counts registered worktrees into `evidence`, which feeds `plan_id`; B9 had no route for a runtime that drifted after B2 | Resolving sync conflicts by hand, which leaves tool output with an unreviewed edit. Stopping on every `plan_id` change, which makes sibling worktree churn cost a full rebuild. Leaving drift to B9, which has no suspension after the merge. |
| D14 | After the push, a ship-review fix commit may follow the adoption commit only if it adds or deletes no path under the three legacy trees and changes no path the adoption wrote (the contract, `.gitignore`, the sentinel, the map, the record and the projections). Any other needed fix stops before merge with the issue open (amends D3). | Plan review (Claude fallback): ship-issue can push review-fix commits, and the push guard forbids the reset that D3 relies on before push. AC2 constrains the apply's commit, not the later history. `verify` reads the evidence record, not the branch tip. | Forbidding every post-adoption commit, which turns a review typo into an abandoned run. Allowing any fix, which could hand-edit tool output or reintroduce legacy paths. |
| D15 | The first B7 stale check found `origin/main` at `763465f` (#176) adding legacy-tree paths, so the owner reset to `BASE` and restarted at B1 (D5). The re-sync conflicted in `CLAUDE.md` and `docs/standards/agent-helpers.md`, where every conflicted line changed on one side only. The merge keeps Task 1's re-points and #176's prose verbatim. #176's new lifecycle-guard link is the one living legacy reference B1's grep found, so it returns to the loop as Task 2 (D10). Task 2 runs on the model its trailer names, so the Global Constraints' line stays truthful. | D5; D10; D13; ship-issue SYNC.md (the escalation format is a self-answer under `--auto`); Task 1's deferred trailer finding | Re-pointing inside the merge commit, which leaves the edit unreviewed. Hand-editing a hunk that changed on both sides: none exists. Relaxing the trailer constraint, which would change a reviewed constraint for one task. |
