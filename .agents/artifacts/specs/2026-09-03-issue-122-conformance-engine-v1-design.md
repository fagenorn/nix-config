...
| Path | Disposition |
|---|---|
| `home/common/agent-skills/scripts/conformance.py` | new — the entry module: the CLI, the single exception boundary, report assembly and output |
| `home/common/agent-skills/scripts/conformance-registry.py` | new — the closed vocabularies, the check declarations, the purpose table and the repairs |
| `home/common/agent-skills/scripts/conformance-checks.py` | new — the evaluators |
| `home/common/agent-skills/tests/conformance_test_support.py` | new — the fixture builders, the hermetic runner and the loaders the three suites share |
| `home/common/agent-skills/tests/test_conformance.py` | new — the report schema, the resolver ladder, the host domain and the boundary |
| `home/common/agent-skills/tests/test_conformance_checks.py` | new — the repository- and verification-domain evaluators |
| `home/common/agent-skills/tests/test_conformance_registry.py` | new — registry closure, purpose selection and the committed-root acceptance gate |
| `home/common/agent-skills/default.nix` | modified — install `.agents/bin/conformance` executable, with its two sibling modules beside it unexecutable |
| `justfile` | modified — add the three new suites to `agent-workflow-tests` |

Nine files, not the four D1 first drew: the engine and its suite each ship as three and four co-located modules for the packaging reason D40 records. Nothing else changes. In particular `.agents/project.json` gains no `commands` entry and `.agents/instructions/bootstrap.md` is untouched (D12, D13).
...
| D39 | D18's root-bounded walk is defense-in-depth, not a currently falsifiable invariant: the bound stays in the evaluator and is cited there, and no test claims to prove it | `discover_root` resolves the root before any evaluator sees it — `Path(repo_root).resolve()` for an explicit `--repo-root` and `Path.cwd().resolve()` for the walk-up branch — so every ancestor symlink is already collapsed and no reachable runtime behaviour distinguishes a bounded walk from an unbounded one. D18's premise (macOS `/tmp` is a symlink to `private/tmp`, so an unbounded walk would fail every fixture) does not hold against a resolved root. The bound stands so the evaluator remains correct against a future resolver that stops resolving | Adding a test seam that hands the evaluator an unresolved root — the plan fixes the three test seams and a task needing a new one is a plan bug, and S3 is reserved for the top-level `except Exception` wrapper and closed-set `raise` branches; or deleting the bound — it would have to be reinstated the moment the resolver changed |
| D40 | The engine ships as one entry module plus two sibling modules co-located exactly like `resolve-project` — `scripts/conformance.py` (CLI, the single boundary, report assembly and output), `scripts/conformance-registry.py` (closed vocabularies, check declarations, purpose table, repairs) and `scripts/conformance-checks.py` (the evaluators) — each installed extensionless under `.agents/bin/` and loaded through the D2 `SourceFileLoader` helper generalised from `load_resolver`; the suite splits the same way. This supersedes D1's single-file architecture and the plan's four-file constraint; no observable behaviour, report shape, check id, reason code or exit status changes | the finished engine is 77642 bytes and its suite 77844, so each one's whole-file diff (79697 and 79762 bytes) exceeds the review-package `member_max_bytes` ceiling of 65536 on its own. `review-package` splits a diff only at file boundaries, its generated-evidence substitution covers EF Core designer files alone, and its adaptive-context remediation applies only when the violation set is a subset of `member_count`/`aggregate_bytes` — and reduces nothing for a branch that adds every file. The mandatory final two-axis review therefore could not be packaged at all, on either axis: a single-file architecture that cannot be reviewed is not deliverable | Raising `member_max_bytes` in `home/common/agent-skills/artifact-budget-policy.json` — the ceiling is a repo-wide bound on reviewer context and widening it to fit one branch is self-serving; adding the decomposition as an eighth plan task — the implementation-plan package is already at its 8-member ceiling with 60 bytes of aggregate headroom, so the plan cannot carry another member; and moving the modules to `.agents/lib/python/` behind a shim like `artifact-budget` — it puts the entry module in a different directory from `resolve-project` and so changes D2's sibling lookup, trading a packaging change for a behavioural one |
| D41 | `bound_facts` bounds every value **first**, de-duplicates the bounded strings preserving sorted order, and only then takes up to `limit`: the list holds at most `limit` *distinct* entries of at most MAX_FACT_STRING characters, and the companion `count` fact still reports the true total | slicing before bounding let offenders sharing a prefix longer than MAX_FACT_STRING collapse into one repeated string — 12 files under `.agents/scratchpad/<six 40-character segments>/` reported `count: 12, listed: 8, distinct: 1`, spending the whole eight-slot budget naming one subject. D9's ceiling and D30's single route cap the list without ever saying its entries must be informative, and the two tests guarding the bound asserted only a length of 8 and a character count of 200 — both green for eight identical strings | Leaving it — the eight slots are a reader's entire view of the offenders, and one subject repeated eight times hides the other eleven; or truncating from the tail instead, which makes the common prefix invisible rather than the leaf and stops the entries reading as paths at all |
