...
| D24 | from-issue's two refusal sentences — Phase 7's in `SKILL.md` and the ledger-only bookkeeper's in `AUTO.md` — name the supersession re-entry line literally as `` `/from-issue <num> --auto` `` printed on its own line, the spelling `ship-issue/SKILL.md`'s `## Launch guard` already uses, and each is pinned by a contract assertion in `test_workflow_skill_contracts.py`. | Inside `from-issue/SKILL.md` the bare phrase "the canonical re-entry line" has exactly one other binding — the `## Suspension procedure`, whose canonical line is the `Suspended (blocked_on=…)` banner and cannot be produced without a `workflow-state suspend`: a ledger write, and in the resume shape the D8-forbidden parking of the successor's live attempt. The two halves of one refusal shape therefore disagreed, ship-issue spelling the line out and from-issue pointing back at a suspension. Surfaced as a cross-task gap by Task 5's review, spanning Tasks 3 and 5, and confirmed by the controller against the live file. | Leaving the bare phrase as the plan prescribed — it routes the most exposed reader, the ledger-only bookkeeper (a cheap agent told to run an exact sequence and nothing else), toward the exact write the guard exists to prevent, and Task 5's `assertNotIn("workflow-state suspend", …)` guards are structurally blind to it because the cross-reference lives 180 lines earlier. |
| D25 | `parse_action_id` bounds the ordinal grammar itself — `ACTION_ID_PATTERN`'s three runs become `[1-9][0-9]{0,17}` — so an absurd-but-well-formed action id is the controlled `WorkflowError("invalid action_id")` at exit 2, the same refusal `14:1` already gets, rather than an uncaught `ValueError`; and `ship-issue/SKILL.md`'s `## Launch guard` states in its one-rule paragraph that the rule binds regardless of `issueTracker.kind`, so a `kind=none` invocation's bare `origin` push is guarded too. Both are pinned: a new oversized-ordinal row in `test_check_launch_separates_well_formed_negatives_from_errors` and a new `collapsed` assertion in `test_ship_issue_guards_every_pre_merge_forge_write`. | The whole-branch final review's correctness axis (Codex) found that `int()` raises past CPython's 4300-digit integer-string conversion limit while the pattern accepted unlimited digits; reproduced live at exit 1 with a traceback, violating Task 1's stated invariant that every answer is exit 0 and every error is exit 2 with empty stdout, a `workflow-state: <message>` stderr line and no traceback. Separately, two independent reviewers read Phase 4's `issueTracker.kind=none` skip — which sits above both of its `check-launch` pointers — as reaching a bare `origin` push without passing a guard pointer. | Catching `ValueError` at the call site, or raising the limit with `sys.set_int_max_str_digits` — both leave the grammar claiming to accept identities the parser cannot represent, and put the bound somewhere other than the one home of this grammar (per D6). And stating the `kind=none` coverage in Phase 4's opening instead, which would disturb the `assert_ordered` assertion pinning that phase's two guard calls to their two forge writes. |
