...
- with **no** free slot the sweep demotes the attempt to `suspended(unknown)`
  and returns idle; a later eligible sweep resumes it in place — the specified
  behaviour;
...
- **`orchestrate-issues`' final-report section**, where the dispatcher renders
  ...
  a later eligible sweep resumes, and never by `retried` or `retry_refused`
  (per D16 — a suspension arms no deadline, and a sweep still resumes only once
  the tracker, capacity and the recorded worktree allow it).
...
| D15 | The whole-change verification gate is spelled so it can actually fail: the permission-guard suite is produced with `just show-claude-settings` and bound through `CLAUDE_SETTINGS_PATH`, every gate that pipes a runner into `tail` runs under `set -o pipefail`, Task 4's absent-at-base proof is scoped to the same section the contract test scopes to, and Task 1's red-phase expectation names six guaranteed failures instead of nine. | Standards review (Codex, base `0fda99d`). Each was a gate that could not do its job: `tests/test_claude_permission_guard.py:10` reads `os.environ["CLAUDE_SETTINGS_PATH"]` at import, so the bare invocation raises `KeyError` before collecting a case; a `runner | tail` pipeline exits with `tail`'s status, so a red suite reads as green; `"consumes no attempt"` already appears at `from-issue/SKILL.md:296` in the suspension procedure, so the file-wide probe aborts Task 4 before it starts; and the base already routes the no-dispatch expiry correctly (`workflow-state.py:1921`), so two of the five new cases are baseline-preserving coverage that is green by design. the-bar *Tests that can fail*. | Leaving the gates as written and letting the executing agent discover each failure — three of the four abort or mislead a task before its real work begins, and the fourth teaches the implementer to distrust a correct green. |
| D16 | The orchestrate-issues final-report prose says a parked expiry is resumed by **a later eligible sweep**, not by "the next sweep", and the contract-test anchor moves with it. | Standards review (Codex, base `0fda99d`). A suspension arms no deadline — `command_control` builds `deadlines` only from `active`/`handed_off` attempts — so the sweep that parks the attempt renders `finalize` and there may be no next sweep at all; and a sweep that does run still returns a `terminal` instead of resuming while `tracker_halt_reason` holds, and stays parked without capacity or an observed recorded worktree. Promising "the next sweep" would make the dispatcher's own final report state something the policy does not guarantee, which is the class of defect D8 exists to prevent. | Keeping "the next sweep" and treating the gap as a rounding error in prose — it is the sentence a human reads to decide whether an issue is still moving. Enumerating every eligibility condition in the shipped sentence — accurate but unreadable, and it duplicates policy that D2 already owns. |
