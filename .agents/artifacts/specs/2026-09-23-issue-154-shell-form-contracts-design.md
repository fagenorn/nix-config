...
(#171's lifecycle calls) D23–D25, the plan amendment D26, later reviews
D27–D28, the ship-time syncs D29–D30, and the ship review D31.
...
- **The one sanctioned chain.** The `unset GITHUB_TOKEN && ` prefix that
  `ship-issue/SKILL.md`'s gh hygiene derives from
  `bindings.tracker.credential_env.unset_before_invocation` stays exactly as
...
assignment in the guard's own source, `home/common/claude-code/lifecycle_guard.py`
(D30), and fails loud unless exactly one exists. `unset GITHUB_TOKEN; gh …` or any other spelling is still a
`chain`.
...
| D3 | "Same parser/checker boundary as production" means one public `refused_examples(document_text)` that the source sweep, the installed sweep and every fixture call; its scanner is purpose-built and quote-, substitution- and heredoc-aware (the guard's move to its own source is D30) | #153's one-function boundary; the-bar Tests that can fail; the harness checker is not in the repo; stdlib `shlex` treats `"$(cat <<'EOF'` as one quoted word; the lifecycle guard's tokeniser lived in a Nix string at design time, drops heredoc bodies and treats `\|` like `;` (the-bar Framework-first, checked) | Reuse the guard tokeniser — not importable without a build at design time (D30), and built to find verbs, not forms; regexes over raw lines (retained) — cannot tell quoted text from live shell |
...
| D28 | An inline code span is read as CommonMark pairs it: its closing backtick may sit on a later line of the same prose block (never across a blank line, a fence line, or into or out of a heading, table row or new list item), its line breaks read as spaces before placeholders are substituted, and each finding keeps the document line of its operator (`unparseable`: the span's first line); an opening backtick with no closer in its block is literal text, not a span (refines D2, D19; reverses the plan's single-line span pattern) | Final correctness review C-I1: `refused_examples` returned `()` for a span holding `git worktree list` then `\| grep x` across a line break; about 20 command spans in the swept trees already wrap, so a wrapped operator would pass both sweeps; the spec's "Inline code spans in prose" names spans, not lines; the-bar Fail loud | Keep single-line spans — a wrapped pipe or chain passes silently and an unclosed line-end backtick pairs with the next line's; report every span left open at a line end as `unparseable` — reds about 20 clean live spans and forces prose rewraps outside scope |
| D29 | The ship-time sync with `origin/main` at `12de02d` adopts the strict project resolver's (#100) retained-binding spellings wherever this slice's text or fixtures named a retired binding: the guard-form `gh pr create` reads `--repo <resolved-repository> --base <integration>`/`<integration-branch>` `--head <branch>`, the pre-flight prefix reads `<bindings.vcs.worktree.prefix>`, D18's historical offenders keep their shell form but take the same placeholders, `COMMAND_VOCABULARY` retires `resolve-bindings` and admits `adopt-project`, and D5's prose home moves to `ship-issue/SKILL.md`'s gh hygiene because #100 removed `from-issue/bindings.md`'s tracker-cli hygiene paragraph | #100's `test_living_source_has_no_legacy_policy_surface` forbids the legacy names in living source, fixtures included; the offenders' refused forms, not their placeholder spellings, are what D18 pins; SYNC.md's retirement sweep | Keep the legacy spellings behind `# policy-gate-pattern` markers — the marker is honored only in the policy-gate test files; leave the pointer at `from-issue/bindings.md` — it no longer spells the literal |
| D30 | The ship-time sync with `origin/main` at `763465f` follows #176's extraction of the lifecycle guard: the sweep reads D5's literal from the single `UNSET_GITHUB_TOKEN_PREFIX` assignment in `home/common/claude-code/lifecycle_guard.py` and D23's whole-allowed helpers from the allow list that stays in `home/common/claude-code/default.nix`, one source constant each, both still fail-loud; D3's rejection of reusing the guard tokeniser stands on its remaining reasons (refines D3, D5, D23) | #176 (`a084c72`) moved the assignment into the guard's own standard-library source, so the retained read of `default.nix` found zero assignments and failed loud at import; the `"Bash(<word>:*)"` entries did not move; the guard's `split_segments` still drops heredoc bodies and splits on `;&\|` alike, so it cannot tell the four forms apart; SYNC.md's retirement sweep | Keep one source and read both from `lifecycle_guard.py` — it holds no allow list, so D23 would fail loud; restate the literal in the test now that the guard moved — the second home D5 rejects; import the guard's tokeniser now that it is a plain file — it still finds verbs, not forms |
| D31 | ship-release's single-branch range writes the previous tag into the follow-up command as `<prev-tag>` (or omits the range for a first release) instead of expanding `${PREV:+$PREV..}` from a variable an earlier call set, in both the Phase 0 pre-flight and `CHANGELOG.md`'s step 1 (refines D10) | Ship-review correctness I2: D10 already says agents run one call at a time, and a fresh shell leaves `PREV` unset, so the verbatim `git log ${PREV:+$PREV..}origin/<default>` reads the whole history, is never empty, and skips the nothing-to-release stop; the-bar Fail loud | Keep the expansion and add a `PREV=$(…)` step — the assignment still dies with its call; restore the one-call `PREV=$(…) && git log …` — the refused chain D10 removed |
