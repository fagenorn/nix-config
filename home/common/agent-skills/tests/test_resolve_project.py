...
class UsageErrorTest(ResolverTestCase):
    ...
    `write-projections` left this list when Task 3 registered it (D18); only
    the subcommand no task has implemented yet, and the empty argv, remain.
    """

    def test_an_unregistered_subcommand_exits_two_without_json(self):
        for args in (("check-projections",), ()):
