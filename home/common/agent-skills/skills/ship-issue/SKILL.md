...
- The branch diff is small: ≤400 changed lines AND ≤20 files, counted with `git diff --numstat $BASE_SHA..$HEAD_SHA` (lines = additions + deletions summed, files = row count) after dropping rows matching the lockfile allowlist, carrying a generated header (`<auto-generated>` / `// Code generated by`), or under this run's own `<specDir>`/`<planDir>` artifacts — the gate measures PRODUCT changes, not process artifacts. Historical artifacts that are themselves the requested product still count.
