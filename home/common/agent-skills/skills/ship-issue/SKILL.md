...
- The branch diff is small: ≤400 changed lines AND ≤20 files. Mechanics: count
  ...
  lockfile allowlist patterns, whose file carries a generated header
  (`<auto-generated>` / `// Code generated by`), or whose path is this run's own
  `<specDir>`/`<planDir>` artifacts — the gate measures PRODUCT changes, not the
  pipeline's process artifacts. Historical artifacts that are themselves the
  requested product (the issue asked for that document) still count.
...
`--delete-branch` may fail or silently no-op on the remote branch while the worktree still has it checked out. The remote merge still succeeds. After verifying the merge, ask the REMOTE whether the branch still exists — `git ls-remote --heads origin <branch>` (actual branch name, including `branchNaming.worktreePrefix` if present); PR metadata like `headRefName` is retained after deletion and proves nothing. Non-empty output → `git push origin --delete <branch>`.
