...
review-package's `delivery-detail` mode. Supply issue/branch/run/head
identity only; the producer derives the per-run leaf beneath the primary
checkout's `.superpowers/issue-delivery/` home and enforces no-clobber
publication.
