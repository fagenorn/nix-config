...
**Claude Code is declaratively managed** by `home/common/claude-code/default.nix` — read it before touching any Claude Code config on this machine:
  ...
  - Use `just show-claude-settings` to build and print the settings JSON (failing unless discovery finds exactly one artifact); its 16-entry allow surface permits branch deletion only as `git branch -d <branch>` and PR merging only as the guard's repository-bound `gh pr merge` subset, through a fail-closed `PreToolUse` hook that checks the live `fagenorn/nix-config` repository, an open `main`-targeting PR, and `main` protection requiring `Nix Eval` with admin enforcement; bare `Agent` remains inert under `defaultMode = "auto"`; verify the built artifact and guard with `tests/test_claude_permission_guard.py`.
