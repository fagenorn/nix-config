...
| D10 | The Phase-5 standards review ran as the **native Claude reviewer**, not Codex, despite `codex.planReview.enabled` defaulting to true | The attempt deadline (2026-08-17T10:46:07Z) left ~65 minutes when Phase 5 opened, and `codex-collaboration` budgets up to ~15 minutes for one isolated pass while Phases 6-7 still need ~40; the skill's own guidance is to fall back rather than wait. The slice is documentation-only, where the correctness axis Codex adds has least purchase. A deliberate timebox fallback, not a Codex failure | Wait for the Codex pass - risks the attempt expiring mid-execution, the failure mode D24 of the degradation-gate design spec records |
| D11 | The item-4 contract pin asserts the whole clause `the Phase-0 number is an *estimate*`, not the bare token `estimate` (refines D6) | Standards review, verified empirically at `de83938`: `investigate.md`'s step 5 already reads `**Scope-size estimate**`, so a bare-token pin passes at the base commit and leaves the estimate half of the estimate/count split unguarded - a vacuous subtest inside an otherwise fail-at-base method | Keep the bare token - passes at base, so it guards nothing; scope the match to the C4 line with the suite's `self.section` helper - narrows it correctly but adds a dependency the six sibling fragments do not need |
| D12 | Item 3's edit re-flows the **whole** `Reading it the way the gate does: ...` paragraph, and D4's snapshot-untouched gate is phrased against the fenced blocks and the two surrounding paragraphs rather than a line count | Standards review: re-scoping the paragraph's first sentence at the file's ~90-column width moves all eight of its lines, so a gate expecting only the re-scoped sentence's original wrapping would read a correct application as a violation. What D4 actually protects is the recorded measurement - the fenced command, the fenced `64 lines` output, and the `Recorded 2026-08-17` / `Re-run this command fresh at ship time` paragraphs | A ragged partial re-flow keeping five lines byte-identical - buys a smaller diff at the cost of an orphaned short line mid-paragraph |
