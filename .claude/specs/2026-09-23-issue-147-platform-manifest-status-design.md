...
| D12 | The final review's correctness finding (the installed-manifest loader accepted bare `NaN`/`Infinity`, so `platform-status` could stream a partial object followed by an error object) is fixed in one separate commit after the reconciliation commit: `agent_platform.load_manifest` refuses the three non-finite tokens as its existing `platform.manifest.parse` refusal. The nine picks stay verbatim, and no repair id is added. | D1 ("all new work goes in separate reconciliation commits"); the resolver's contract loader already refuses these tokens through `reject_non_finite`, and `emit_json`'s comment promises the parse side does; RFC 8259 has no non-finite numbers, so `parse` names the condition truthfully | Deferring the fix to #148, which ships a known way to print unparseable output. Editing a recovered commit, which D1 rejects. A new `platform.manifest.non_finite` repair id, which widens a closed set. |
