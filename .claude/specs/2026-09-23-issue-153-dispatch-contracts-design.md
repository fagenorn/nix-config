...
retiring the legacy bindings fallback). Decisions D1–D16 bind the plan.
...
| D15 | `missing_contracts` returns the ids whose clause does not occur exactly once in the rendered region (absent or duplicated). The mutation matrix adds, per carrier × contract, a duplicate inside the region and, for each `prompt: \|` fence, a relocation into the dispatch header, each yielding exactly `{that contract}`; its outside-region relocation re-inserts the clause at the document's top. Refines D6, D10, D14 | Exactly-once rule and the one-function boundary; D10 independence; D14 is unfalsifiable unless a header-placed clause reds; every region opens after a fence, anchor or heading line, so the top is outside all of them | Raise on a duplicate — reds both contract methods for that carrier; a second public counting function — widens the boundary; end-of-file relocation — inside a section region whenever that section is the document's last |
| D16 | `AGENT_SKILLS_INSTALLED_HOME` must name an absolute directory; an empty or relative value fails, never skips; refines D8 | D8 "set variable … fails"; an empty value resolves to the cwd, and a checkout holding `.claude/skills` and `.agents/skills` would pass with nothing installed checked | Treat empty as unset — a mistyped recipe variable silently skips; resolve a relative value against the cwd — the verdict depends on where unittest runs |
