...
Design, 2026-09-24. Interactive `/design` session: the user answered or
accepted every row from D1 to D15, and D16 came out of #174's review. Sliced
into #175 (foundation), #176 (guard), #179, #177 and #178 (cluster moves).

Earlier accepted rows this spec supersedes or refines. Every other row of
these specs still binds:

- **resolver v1 D1** — refined: the `resolve-project` command and its
  `~/.agents/bin` path stay; its source moves into the package (D7).
- **conformance engine v1 (#122) D2** — superseded: the engine imports the
  resolver as a package module, not through `SourceFileLoader`. **D40** —
  refined: the three-module split stands; extensionless co-location and the
  loader helper go (D2, D12).
- **artifact-budget policy D8** — refined: the `artifact-budget` command and
  the `~/.agents/share` policy path stay; the importable
  `~/.agents/lib/python` copy goes (D12).
- **delivery reconciliation (#151), "Shared delivery model seam"**, and the
  lexical build-module load in **delivery contract source (#171) D3** —
  superseded in mechanism: lexical source/installed loading and the
  `*_INTERFACE_VERSION` handshakes give way to package imports under isolated
  launchers (D2, D12). The fail-closed intent is kept, and now holds by
  construction. The model's public surface otherwise stands.
- **#171 D27** — refined: `build-delivery` still runs `resolve-project resolve`
  itself, but reaches the packaged resolver instead of a sibling file.
- **model/effort drift (#98) D13** — refined: the sibling-module split stands;
  its modules import each other as package modules.
- **platform manifest status (#147) D7 and its inherited D23** — refined:
  test `HOME`s still hold the manifest, but no longer the library.
...
- **Knowledge is copied.** The telemetry digest (agent-cost-telemetry D9:
  `sha256:` over sorted, compact, ASCII-escaped JSON) is copied word for word
  into four modules across three tools, and the duplicate-key JSON hook into
  three more. A second,
...
`~/.agents/lib/python` tree are deleted. The telemetry digest gets one home. The
...
- **Members.** Every deployed helper — the agent-skills scripts,
  `review-package` and `context-map-lint`, each linked into `~/.agents/bin`
  today — and every repo-only agent tool (`agent-costs`, `agent-gate-bundle`,
  the `agent-model-drift` family), meaning a tool with no `~/.agents/bin` link.
  Module names are the command names with underscores. Deployed helpers get
  launchers; repo-only tools are modules without one.
...
checks against them, the `~/.agents/lib/python` tree, the two unexecutable
conformance library entries in `~/.agents/bin`, the `artifact-budget` shell
wrapper (replaced by a launcher), and the tests that exist only to
...
A shared `canonical` module owns the telemetry digest (sorted keys, compact
separators, ASCII-escaped, no trailing newline, `sha256:` prefix) and two strict
JSON load hooks: one rejects duplicate keys, the other rejects the
NaN/Infinity literals. The four telemetry-digest copies and the three
duplicate-key hooks import it. Each caller composes exactly the hooks it
applies today, so no command changes what input it accepts (D15): the
gate-bundle tool keeps rejecting those literals, while the evidence validator
and the model matrix keep accepting them. The delivery model keeps its own format
...
The guard becomes a standalone Python file, `lifecycle_guard.py`, beside the
claude-code module's Nix file.
...
`agent-model-matrix`) switch to `python3 -m`. Suites that build a child
environment from scratch, such as the conformance suites' hermetic
environment, forward the recipe's `PYTHONPATH` into it explicitly. Nothing in
Python, tests included, edits `sys.path`. A test file run outside a recipe fails with
`ImportError`. Test files stay where they are.

### Cross-command calls (D16)

Some modules find another command by a path derived from their own file and
run it as a subprocess: `workflow-state` runs the budget validator and
`resolve-project`, and `review-package` runs `sdd-workspace` and runs the budget
validator as a script. From here on, a packaged module runs a packaged sibling
through one package helper. The helper builds `[sys.executable, "-I" only when
the caller itself runs isolated, "-m", "agent_tools.<module>"]`, so the child
resolves the same package the parent did, in both the installed layout and the
source layout. An executable outside the package (the sdd shell scripts) runs
by its command name on `PATH`, and tests put the source copy on `PATH`, as the
`task-brief` suite already does for `artifact-budget`. Nothing locates code
from `__file__`.

A caller and a callee it locates by path move in the same PR. When a slice
moves only the callee — the resolver slice moves `resolve-project` while
`workflow-state` waits for the delivery slice — that slice gives the caller a
transitional lookup that works in both layouts, and the slice that moves the
caller deletes it.
...
`agent-helpers.md` shard — and it is added to the project policy's standards
paths. The shard governs `python/**` and the guard file. It also governs the
legacy script directories (`scripts/**`, `home/common/agent-skills/scripts/**`,
`home/common/agent-skills/skills/*/scripts/**`), so a new flat script still
loads the rule that forbids it. The shard holds deltas only:
...
3. No import machinery and no path-derived calls: no `sys.path` edits, no
   `importlib` loaders, no module version handshakes, and nothing locates
   another module or command from `__file__`. Sibling commands run through
   the package helper (D16).
4. Digests use the shared telemetry digest (sorted, compact, ASCII-escaped
   JSON, `sha256:`-prefixed) or the delivery model's own format, never a
   local copy.
...
1. **Foundation PR:** the `pyproject.toml`, `lib/agent-tools.nix`, the launcher
   ...
   modules that copy the telemetry digest or the duplicate-key hook moved in (two of which have
...
1. **Command contract and module interface, from source.** Each command's
   argv/stdin → stdout/exit contract, run as `python -m agent_tools.<module>`
   with the recipe's `PYTHONPATH`. Also the public interface of the pure
   modules the existing suites already import directly, such as the delivery
   model facade. These are the existing suites. Only how they locate code
   changes, plus forwarding `PYTHONPATH` into any child environment a suite
   builds from scratch (D8).
2. **Installed layout.** A new test inside `just agent-installed-skill-tests`,
   ...
   `AGENT_SKILLS_INSTALLED_HOME`. It runs every launcher the command table
   generates under that tree's `.agents/bin`, proves each one reaches its module's own argument handling
...
- The shell scripts (`task-brief`, `sdd-workspace`) and their inline Python
  heredocs.
...
|----|--------|-----------|----------------------|
| D1 | Package members: every deployed helper plus the repo-only agent tools; the guard is handled by D4 | Telemetry-digest copies live in the repo-only tools; user, round 1 | Deployed helpers only — leaves the duplication in place |
...
| D9 | Three seams: command contract and module interface from source, installed layout (with hostile `PYTHONPATH`), the guard's registered hook | `agent-installed-skill-tests` and `CLAUDE_SETTINGS_PATH` precedents; the-bar Tests that can fail | Keeping the fake-`HOME` loader tests — they test deleted machinery |
...
| D11 | Shared `canonical` module owns the telemetry digest + the duplicate-key and NaN/Infinity load hooks, composed per caller as today; the delivery format stays in the delivery model under a distinguishing name | the-bar DRY (dedupe only what must change together); sealed ledger digests | One parametrized function — an option set nobody needs; merging formats — changes sealed digests |
...
| D16 | A packaged module runs a packaged sibling through one helper (`sys.executable`, the caller's own `-I`, `-m`); out-of-package executables run by command name on `PATH`; nothing locates code from `__file__`; a callee moved ahead of its caller gets a transitional lookup that the caller's move deletes | Correctness review of #174: `workflow-state` and `review-package` locate siblings by path | Keep the path lookups — they break once modules leave the flat layout |
