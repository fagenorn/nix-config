...
| D4 | Make the survey step a read of the stat and changed-file sections the packet already includes, not an instruction to run `git diff --stat` | Both lightweight branches already emit stat + changed-files sections, for branch and working-tree targets alike | Instruct a fresh `git diff --stat` — one more app-server tool call for material already in hand |
