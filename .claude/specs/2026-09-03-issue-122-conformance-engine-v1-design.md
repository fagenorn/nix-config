...
`repository.contract.resolvable` is the first check and depends on nothing. Its body runs that whole ladder **once**, inside a single `except Exception`, and caches each stage's outcome; it fails only when an exception that is not the resolver's `ContractError` escapes, and it is the only check that can carry `resolver_failure`. Every other structural check is then a pure read of the cached stage outcome, and is `suppressed` when `resolvable` failed. The ladder is never run twice and no check re-enters the resolver (D17).
