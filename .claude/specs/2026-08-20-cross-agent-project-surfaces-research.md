...
|---|---|---|
| Instructions | **Present** — repo-root `CLAUDE.md` in all three repositories: `CLAUDE.md`; nodo `CLAUDE.md` (`git ls-files -s` → mode `100644`) @`7a3dab7e54`, 2026-09-02; argus `CLAUDE.md` (mode `100644`) @`20d6655223`, 2026-09-02 | **Present** — repo-root `AGENTS.md`: nodo `AGENTS.md`, tracked at mode `120000`, a symlink to `CLAUDE.md` @`7a3dab7e54`; argus `AGENTS.md`, mode `100644` @`20d6655223`. In nix-config the file arrived *during* this study: absent when this matrix was observed, then added at mode `100644` by `512d096` (2026-09-02, issue #118) as a generated projection of `.agents/instructions/bootstrap.md` — see the Instructions/Gaps row |
...
| Gaps | The two agents read **two different filenames at the same root**, so a single instruction body at project scope is impossible without a projection at that root. nix-config, the repository that authors both agents' user-scope trees, had no project-scope `AGENTS.md` when this matrix was observed, so a Codex session working in it inherited user-scope guidance only. Later that same day `512d096` added one — a generated projection of `.agents/instructions/bootstrap.md`, whose own header reads "generated by resolve-project … Do not edit; edit the source". That is this gap's first observed in-repo answer, and it answers it exactly the way this row says it must be answered: by a projection at the root, not by a shared file | Same gap, from the other side. Additionally, the only *validated* project-scope projection observed is nodo's git-tracked symlink; argus's pointer is authored prose with **no observed mechanism that detects drift** between `CLAUDE.md` and `AGENTS.md` |
...
1. **The dogfood target exposes the thinnest surface of the three.**
   ...
   `.claude/plans/` and `.claude/specs/` — as observed it had no `AGENTS.md`
   (`512d096` added a generated one later the same day), no
