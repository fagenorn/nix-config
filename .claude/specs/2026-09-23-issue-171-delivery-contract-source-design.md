...
| D40 | Narrows the adapter's per-issue contract rule (refines D31): on the first sweep, a contract is built for an issue without a bootstrap requirement only when this invocation created the run (`init-run`), and for every requirement whose `contract_digest` is null (from its `recorded_worktree`). On a reused run, an issue without a requirement is sent null until its latest summary carries `delivery_contract_required`. from-issue's durable interactive route follows the same rule | Task 7 review (I3, plan-mandated): bootstrap omits delivery-complete issues, and a fresh contract for one differs from the installed one (new `created_at`, a candidate path), so control's immutability check exits 2 for the whole sweep and strands every re-invocation of a reused run (the class of defect D34 fixed). A fresh run cannot hold a delivered issue, and control already asks a never-seen issue for its contract | Bootstrap marking delivery-complete issues (widens PR 1's merged wire shape); sending null first for every issue (costs every fresh run a sweep) |
| D41 | Task 7 re-pins the `Earlier controller stop` contract test to `--boundary workflow-response`, notwithstanding its brief's "no other test changes" | Task 7 review (I4, plan-mandated): the delegated owner returns the durable `finish` reply, a workflow-response, and AUTO.md says so. The old pin matched only a negation clause and would still pass if the rule regressed to `ship-summary` | Keeping the pin (it asserts nothing) |
| D42 | A remainder owner that finds selection pending builds it from the PR head with literal refs: `acceptance_ref` is the issue URL, `review_ref` is `unknown` and `test_ref` is `checks`. A relaunched remainder owner therefore re-derives the identical selection | Task 7 review (W1): a reconciled remainder (D30) can start with `select_reviewed_output` pending, yet it has no spec root or review report to use as the owner's fixed refs, and neither §6 nor D5 names any. `unknown` is the review state that already means "no evidence of a completed review" | A spec-root lookup by convention (not deterministic across relaunches, and the path may be absent); leaving the refs unspecified |
| D43 | The ship-handoff's `authorization_chain_digest` is sealed by the builder, never composed by an agent. `build-delivery` gains one output kind for it, derived from the intents the owner already holds, and ship-handoff.md uses that output | Task 7 review (W2): no helper output the owner receives carries the chain digest, so the prose had to spell out a hand derivation. That contradicts the spec Solution's "no agent ever composes a digest" and CLAUDE.md's matching claim. A wrong value only fails closed, but a hand-hashed digest is the error-prone step #171 exists to remove | Amending the Solution claim to exempt the chain digest; a hand-derivation paragraph |
| D44 | Closes D36 (refines D25, D34): with no installed contract, a retry or `new_run` whose recorded worktree observation is `mismatch` refuses at once with the D8 custody refusal and no write, in direct and control alike. The candidate fall-through is removed, and a test pins the contractless `mismatch` outcome | Under contract-last acquisition every dispatch needs an installed contract, and D25 binds a retry or `new_run` contract's `remove_worktree` to the recorded path. The fall-through can therefore end only in D8's refusal after a wasted `"contract"` round trip, or in a demand for a candidate the adapter never reports for a recorded issue (D25, D40). The contracted lane already refuses a `mismatch` | Ruling D36 final (keeps the wasted round trip and a candidate demand nobody can answer) |
