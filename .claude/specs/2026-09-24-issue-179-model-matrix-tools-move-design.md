...
| D13 | Extends D6: two regression pins for the hook swaps — the drift reporter's duplicate-key stderr equals `cannot load JSON input`, and the matrix's `validate` reports `duplicate JSON key` for a duplicated matrix key | the-bar Tests that can fail; Phase-5 standards review (Claude fallback): D4's widened clause and the matrix hook are otherwise invisible to every suite, since `main` also catches `ValueError` | No pins (D6's closed list) — dropping either hook or D4's clause would pass every suite silently |
