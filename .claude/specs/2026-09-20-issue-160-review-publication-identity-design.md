...
- Exercise acquisition errors and locally owned descriptor release on success and publication failure; inject a close error after a real close to check that remaining release/restoration occurs and failure is surfaced. Assert the borrowed parent remains usable. Do not claim an artificial close failure proves every kernel's interrupted-close behavior.
