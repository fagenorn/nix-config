...
| D15 | Make the authored package contract pin all seven scan-evidence items in order, the exact report scaffold/CDNs/accessibility fallbacks, and exactly nine ordered adaptations before an exact upstream MIT-notice suffix | Phase-5 B1; parent D3/D4 and issue-spec contract coverage make this suite the only automated authored-behavior seam before activation | Substring spot checks: can remain green after required evidence, offline readability, or provenance ordering is removed |
| D16 | Make the fog eval prove the terminal stop: no worktree, spec, plan, source/test change, or continuation into issue creation, planning, or execution | Phase-5 B2; parent D5 stops after `wayfind`; tracker `kind:none` creates no canonical issue artifact, so repository state plus stop-semantic output are the observable seam | Checking only that a map exists and output mentions `wayfind`: permits a run that charts correctly and then keeps mutating |
| D17 | Permit one implementation-time, read-only authoring fetch from four immutable raw GitHub URLs pinned to revision `9c9f36ccd3995266cd675468af71639c8dde1ec5`; fetched files stay temporary and never become build/runtime/generated inputs | Phase-5 B3; D1/D2/D8 require exact upstream material while the vendored-skill convention forbids automatic synchronization, not a manual pinned comparison | Guessing upstream prose or relying on `/private/tmp`: neither is reproducible for a fresh implementer |
| D18 | Pin each deployed eval assertion by unique name and inspect its `shell` independently, including exact commit, immutability, recommendation, and stop fragments | Phase-5 S1; the-bar tests-that-can-fail requires shell behavior rather than words that may appear only in prompts or assertion labels | Concatenating prompt, name, and shell before substring checks: a `true` shell can false-green on words supplied elsewhere |
