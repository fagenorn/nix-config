...
| D31 | Narrows D26's gate: a later release opens a refusal-gated custody only when its `release_event` is not `launch_refused`; another custody's refusal frees no capacity, so two refused owners cannot wake each other, and the `finalized` release still reopens gated work on the next invocation | Task 5 review (reproduced two-refusal ping-pong to the anti-zombie bound); D7 ("no re-dispatch without an intervening release"); spec validation row "the response to the refusal holds no dispatch for B"; D26 | Any later release opens the gate, `launch_refused` included (refused owners alternate until both stop `stalled`) |
| D32 | The replay's evidence assertions must be able to fail: the 3-slot control refusal pins its `declaration_invalid` reason, and the fixture host refuses only scripted launches so the per-minute declared-agents assertion, not a pre-launch guard, is what would catch an over-admission; baseline values unchanged | Task 6 review (probe: the 3-slot refusal assertion also passed at 4 slots; the host guard made the D27 per-minute check unfailable); D27 ("measured rather than assumed"); D28 | Keep the brief's assertions verbatim (both hold by construction) |
