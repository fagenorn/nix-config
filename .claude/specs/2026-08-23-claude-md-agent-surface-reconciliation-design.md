...
- No mention of Superpowers as a live input, patch, marketplace or plugin. The only
  ...
  paths under `home/common/agent-skills/` are pipeline **state and artifact**
  locations sharing a historical name — the lifecycle ledger under an explicitly
  supplied repository root, per-plan `sdd` artifacts in the current working tree, and
  delivery detail beneath the primary checkout. Written as a fact about what those
  paths *are*, never as a changelog entry about what was removed and when (per D6,
  as amended by D13 — do not describe them as one directory, and do not tie their
  placement to the repo the scripts happen to run in).
...
| D6 | State in the correction, in the present tense, that the `.superpowers/` paths are pipeline state and artifact locations sharing a historical name — a fact about what they are, not a changelog note about a removal. Amended at Phase 5 (per D13): name the three distinct homes rather than one "workflow-state directory created in whichever repo the scripts run in", which was false on both count and placement | `git grep -in superpower` returns ~28 hits, all state paths in `home/common/agent-skills/`; a reader trusting the corrected "Superpowers is gone" would otherwise grep-purge live state | Say nothing (the correction itself creates the hazard, so it carries the guard); write it as removal history (a context document states what is, not what changed — that is the commit log's job); document the state directory fully (scope creep — one clause prevents the wrong edit) |
...
| D11 | Scope the "Test seams" `git grep -in superpower` check: exclude `.claude/plans/` and `.claude/specs/`, leave the why-not comment at `home/common/codex/default.nix:16` in place, and check `CLAUDE.md` by absence of its three exact stale phrases rather than by absence of the word | The check as written is unachievable — the pattern has 120 hits, most in committed point-in-time artifacts, and the corrected clause itself names Superpowers (per D6). `the-bar` "Moves keep their history" keeps accepted records at their original paths, and this repo's module comments routinely carry why-not history (the two `NOTE:` blocks beside `skillsDir` in the same module) | Take the check literally (would force rewriting accepted specs and plans); also rewrite `home/common/codex/default.nix:16` (widens the two-file scope this spec states, to delete a comment that is true and useful) |
| D12 | Ship the plan as a two-member package — root plus `task-1.md` and `task-2.md` — rather than as the single root file the plan phase's brief anticipated for a change this small | `artifact-budget check --kind implementation-plan` hard-requires a `<stem>.tasks/` directory holding at least one member, exactly one `## Task index` heading, and index rows matching its row regex; a root-only plan exits 2 with "cannot inspect member directory" and can therefore never be returned as complete | Single root file with inline task bodies (never validates, so the phase could not return a within-budget artifact at all) |
| D13 | Reverses part of D6: the dictated `.superpowers/` sentence names three distinct homes — the lifecycle ledger under an explicitly supplied repository root, per-plan `sdd` artifacts in the current working tree, and delivery detail beneath the primary checkout — instead of one "workflow-state directory the pipeline scripts create in whichever repo they run in" | Phase-5 review (Codex, B-002), verified live: `workflow-state.py:631` takes `repo_root` explicitly rather than using cwd, `sdd/scripts/sdd-workspace:2-6` places task artifacts in the current working tree, `sdd/SKILL.md:60` places delivery detail beneath the primary checkout, and all three subdirectories exist in the live `.superpowers/` | Ship the one-directory sentence (it is wrong on both count and placement, and would write fresh drift into the very document this issue exists to de-drift); drop the clause entirely (D6's grep-purge hazard is real and still needs the guard) |
| D14 | Test seam: gate scripts live at `.superpowers/gates/` in the ignored working tree rather than under `$(git rev-parse --git-dir)`, and the plan states the gate's proof scope by enumeration — what it pins and what it explicitly does not | Phase-5 review (Codex, B-001 and SF-001). `sdd/scripts/sdd-workspace:11-15` records that Claude Code protects `.git/` and denies agent writes there, which would block the sdd implementer subagent that must run these gates; `.superpowers/` is already ignored via `.git/info/exclude`, keeping the never-committed and never-collides properties. The enumerated scope replaces "It proves the prose agrees with the configuration", which overstated a gate that pins no flake input, patch wiring, or UI/UX destination | Keep the git-dir location on the competing precedent in `2026-08-10-wayfind-skill-hardening.md:108` (that plan's writer was not an sdd implementer, so it never met the protected-path denial); add assertions for every dictated sentence instead of narrowing the claim (unbounded, and several sentences have no machine-checkable referent) |
