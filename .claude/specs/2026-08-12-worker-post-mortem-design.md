...
Add `pruneJobRecordsInStateDir(stateDir)` beside the existing pair convention (readdir → parse → sort newest-first by `updatedAt` → keep `MAX_JOBS` → `removeJobFromStateDir` the rest), and make `pruneJobRecords(cwd)` delegate to it so `MAX_JOBS` keeps one home. Semantics are identical to today's prune, including the pre-existing property that a long-idle *active* job could in principle fall outside a 50-record cap; that risk is unchanged, not widened. A concurrent session's live job is not at risk in practice — its worker stamps `updatedAt` on every progress event, so it sorts newest.

The resulting steady-state ceiling is 50 records plus 50 logs per state dir (one state dir per worktree), where the large records are completed reviews carrying a `result`/`rendered` payload — order-of-magnitude single-digit MB per worktree, against a previous steady state of "whatever the current session has produced so far". That is the cost of the feature, it is bounded by a policy that already exists, and it is the reason the trigger is non-optional.
