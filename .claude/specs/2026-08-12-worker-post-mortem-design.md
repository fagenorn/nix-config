...
- `pruneJobs` caps what any read *sees* — `listJobs` applies it on every call, so no status output and no reader can be flooded regardless of how many files exist. (Corrected twice after this was written: applied status-blind this half is *not* sufficient for AC2's visible surface, because it hides the very record retention preserves. `listJobs` now takes the same `retain` predicate the prune does — see the exemption note below.)
- `pruneJobRecords` is what actually *deletes* over-cap `.json`/`.log` artifacts, and it is reachable only from `saveState` ← `updateState` ← `setConfig`, whose sole production call site is the review-gate toggle. So on-disk deletion happens only when the review gate is flipped — which is precisely why that path has to honour the exemption too: it is a *write* that prunes, and status-blind it deleted the live record outright (corrected at ship).
...
Add `pruneJobRecordsInStateDir(stateDir, { retain })` beside the existing pair convention (readdir → parse → sort newest-first by `updatedAt` → `retain`-exempt split → keep `MAX_JOBS` of the rest → `removeJobFromStateDir` the remainder), and make `pruneJobRecords(cwd)` delegate to it so `MAX_JOBS` keeps one home. (Corrected at ship: `pruneJobRecords(cwd)` was originally specified to pass no `retain`, leaving cwd-scoped pruning status-blind. It threads the caller's predicate instead — `saveState` calls it, so leaving it status-blind meant a config write deleted the exempt record the SessionEnd prune had just kept.)
