...
| D9 | At the pre-terminal checkpoint, seal only the exact rollout prefix through the persisted Phase-5 `delegate` observation; leave both controllers' terminal-prefix digests and maximum-input records to the post-terminal certifier, refining D4 and D6. | Phase-5 reviewer B1 and the deployed relay order show that the earlier controller's terminal endpoint cannot exist until after the fresh owner finishes, ships, and returns the durable terminal bytes. | Require Task 1 to seal the earlier controller's terminal prefix — temporally impossible and risks mislabeling an observation prefix as terminal evidence. |
