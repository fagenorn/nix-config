...
| D8 | Make the migration inventory exhaustive across phase-entry skills, retained-snapshot support documents, `context-map-lint`, and release contracts; assert each configured-review owner/support pair independently; scan only tracked living text; and make the narrow activation-command patch the first mutation immediately before the sole retained resolution. | Phase-5 `fresh-sol-high` review of `c4b4a0d7b9de4feaf8d48d2497f0c1f50f0ae03e` verified that omitted support/helper consumers, concatenated review assertions, unrestricted UTF-8 scans, and a pre-patch test step left the cutover or bootstrap boundary incomplete. | Rely on a top-level skill list, let one review document mask another, recursively decode arbitrary filesystem content, or mutate a test before the one authorized pre-resolution patch. |
