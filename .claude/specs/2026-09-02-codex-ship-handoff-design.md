...
| D17 | The `--auto` pause is split by ledger ownership: a fresh ship owner presents the block and returns a truthful `stopped` ship summary through `artifact-budget validate-report --boundary ship-summary`, keeping the worktree and claiming no merge success, while its `from-issue` parent is what suspends `blocked_on: human_gate` and prints the re-entry line; only a `from-issue` owner walking this path itself suspends directly. Refines D5, which named the right mechanism but addressed the instruction to whoever was running | `ship-issue/SKILL.md`'s launch guard — "A fresh ship owner never writes workflow-state itself; the read-only `check-launch` query is the one ledger call it makes" — makes `workflow-state suspend` a contract violation for the normal ship owner, and `from-issue/ship-handoff.md` requires that owner to return canonical ship-summary JSON, which the suspension procedure's re-entry line is not. `ship-issue/SKILL.md` Phase 8 step 4 already owns the vocabulary for a truthful pre-merge `stopped` row, so the pause reuses it rather than inventing a state | Keeping the single instruction to "follow `from-issue/SKILL.md`'s suspension procedure": at either gate a fresh ship owner would have to either write the ledger it is forbidden to write or return bytes its parent cannot validate, so the promised suspend-and-resume could not happen. Also rejected: relaxing the launch guard so the ship owner may suspend, which trades a prose fix for a hole in the ledger-ownership invariant |
| D18 | The post-grant "nothing further is asked" promise is qualified to the successful path, with the failed execution named as the exception | Same reasoning as D16, applied to the second unqualified claim in the same file: `## Grant semantics` states that "A failed execution is not re-run under the same grant; re-entering the gate is the only path", so an unqualified promise that nothing further is asked contradicts a rule two paragraphs below it and teaches a reader to discount both | Leaving the sentence unqualified and reading it as shorthand for the happy path: the sentence is pinned verbatim by the contract suite, and D16 already settled that prose a test pins cannot be informal |
