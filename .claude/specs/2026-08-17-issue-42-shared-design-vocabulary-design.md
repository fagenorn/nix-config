...
| D15 | Every verification gate fails closed and is evaluated against the working tree: `set -euo pipefail`, unsuppressed Nix stderr, asserted existence of the generation and both parent skill surfaces before absence is claimed, `! grep` for prohibitions, asserted adaptation counts, `--untracked-files=all`, and `git diff <base>` rather than `<base>..HEAD` | Phase-5 review (Codex, isolated read-only) B-001/B-002/S-001/S-002: the original gates piped Nix through `tail` with stderr discarded, printed counts instead of comparing them, inverted their terminal status on a zero-match `grep -c`, and compared committed history only in steps that run before their own commit — so an empty or failed build could print both "ABSENT AT BASE" lines and satisfy two acceptance criteria vacuously | Leaving the gates as diagnostics and relying on the implementer to read the output: the-bar requires verification before claiming done, and a gate that cannot fail is not evidence |
| D16 | The contract pins each canonical term's definition clause, not just its bold heading, and requires every relative link to resolve to a file that stays inside the package | Phase-5 review B-003/S-003: a heading-only assertion stays green with every definition deleted, and a bare `exists()` lets a `../../` traversal pass by reaching a real file outside the package — neither pins what the third acceptance criterion names | Pinning whole paragraphs verbatim: brittle against upstream reflow at the next manual refresh, for no extra protection against the drift that matters |
