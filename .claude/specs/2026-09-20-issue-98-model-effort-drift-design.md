...
Scheduling reports `measured`, `partial`, or `unmeasured` separately. It never changes routing state and has no `waste`, `cheap`, `useful`, `savings`, or billing verdict. The CLI exits 0 only for `conforming`, 3 for `drifted` or `inconclusive`, and 2 for unreadable, malformed, duplicate-key, unsupported-version, unknown-field, or wrong-type input. Valid states always emit one report on stdout; tool failures emit diagnostics on stderr and no report.
