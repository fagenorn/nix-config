...
- **Requested** is a value carried by the exact launch request paired to an execution, including the target host intrinsic to a recognized host-scoped launch transport. It is never copied from configured or observed execution metadata.
...
`source_coverage` contains the authoritative routing aggregate, authoritative per-metric scheduling aggregates, and a `source_only` map. That map is keyed by selected source when the source contributes coverage that no run can carry, including a selected source with zero telemetry runs. Each entry contains routing coverage and the closed scheduling coverage map. Aggregate coverage must equal the deterministic merge of every run contribution and every source-only contribution, including combined reason counts. The consumer validates and projects from that complete set; it never drops a reason because no `run_id` exists.

Scheduling has independent per-metric coverage and nullable values for spawn attempts, capacity rejections, waits, follow-ups, wait input tokens, covered input tokens, slot capacity-seconds, and claimed slot-seconds. The producer emits numerators and denominators, not conclusions. A wait-token share is derivable only when both token counts are fully measured over the same response cohort; occupancy is derivable only when both slot values cover the complete event window. Current sources may legitimately report unsupported fields as `null` with named reasons. A selected zero-run source contributes through `source_only`; full zero-event count coverage may contribute numeric zero, while unavailable coverage contributes null and its reasons. Synthetic scheduler fixtures prove behavior only; they are never cited by the design or report as production measurements.
...
The producer is tested at its existing `main(argv)` JSON boundary with temporary Claude/Codex trees. One fixture proves a fully paired conforming execution and one a hotter-than-declared execution; other fixtures prove implicit transport request host, missing/conflicting explicit request host, event-time exclusion despite recent file mtime, configuration-only Codex evidence, missing role pairing, runtime/model mismatch, missing timestamps, selected zero-run source contributions, and null-versus-zero scheduling coverage. Existing duplicate-response, legacy-reset, attribution, cost, digest, empty-window, and text-byte tests remain the accounting oracle.

The consumer is tested only at its `main(argv)` boundary with strict temporary matrix, baseline, and producer-shaped current/legacy records. Required cases are conforming, requested/observed drift, prohibited and unclassified effort, stale/future/incompatible baseline, missing coverage including a selected source with no run, combined source-only reason validation, host/model mismatch, valid escalation, missing/out-of-scope escalation reason, scheduling absent versus measured zero, component-derived cache availability, and deterministic sorted findings. Each case asserts both stdout shape and exit code; malformed documents assert exit 2 and empty stdout.
...
| D2 | Treat only exact request-to-execution evidence as observed; derive requested host only from an explicit launch field or the intrinsic target of the exact Claude `Agent`/`Task` or Codex `thread_spawn` transport; retain Codex turn/thread values as configured; and select telemetry by an explicit event-time window independent of `--days`. | Current counters are unpaired; current `--days` is file-mtime/whole-file; the truthful-states rule forbids relabelling either. Host-scoped launch transports carry a request target even when their payload omits a redundant host field. | Join run aggregates, backfill request host from configured/observed values, accept a conflicting explicit host, or rename the existing window; each claims evidence the source does not carry or discards a real conflict. |
...
| D5 | Report scheduling coverage and raw cohort-compatible numerators separately from routing; preserve selected-source coverage without a run in explicit `source_only` contributions; null unsupported metrics and derive no waste/savings verdict. | The audit separates scheduling waste from routing drift and rejects cache-read usefulness claims; current sources lack authoritative occupancy. A selected empty source has authoritative coverage that cannot be attached to a run. | Convert missing events to zero, discard zero-run sources, or use cache reads as a waiting proxy; each creates a false production claim from absence or unrelated counters. |
