...
2. **Reconcile and push any pending direct-to-`main` commits — unconditionally, before step 3.** Local
   `main` is one commit ahead of `origin/main` (`c560008`, the out-of-scope record) and step 1 advances
   `origin/main` along this branch's history from the same base, so the two **diverge** and a bare
   `git push origin main` is a non-fast-forward. Fetch, merge `origin/main` into local `main` (never
   rebase, never force — `main` is shared), verify both the PR merge and `c560008` are contained, then
   push and confirm the remote tip. Required status checks apply to
...
> evaluates `nixosConfigurations.anis-desktop` on Linux and is the status check `main`'s branch
> protection requires** — once `just protect-main` has been applied, with `enforce_admins` on,
> `gh pr merge` (including `--admin`) and direct pushes to `main` are
...
| D23 | Local `main` is integrated with the post-merge `origin/main` by a **merge** before the rollout push, gated on `git merge-base --is-ancestor` checks for both the PR merge commit and `c560008`, and protection stays off until the pushed tip matches | Local `main` (`c560008`) and the merged `origin/main` both descend from `b59ff22` but neither contains the other, so the plan's bare `git push origin main` is a guaranteed non-fast-forward. Once protection is on, `enforce_admins: true` plus a required context means that commit can no longer be pushed at all — the reconciliation has exactly one safe window | `git push --force` (rewrites shared history to fix an ordering mistake); rebase local `main` onto `origin/main` (rewrites an already-published-by-merge commit); skip the push and protect first (strands `c560008` permanently) |
