...
The `fleet` block sums the **token** fields of the strata that measured
anything — a stratum whose scan found no runs contributes nothing rather than a
zero (D6, D44) — and is flagged `informative: true`. It carries no cost: summing a list-priced stratum with a
...
1. `unmeasured` if any required evidence is missing, unresolvable, inconsistent
   ...
   than three trials on a side, or unequal `base`/`candidate` lengths; the same
   measurement cited twice on one side, or one measurement cited by two
   different cases; a stratum absent from a case; one of the four core
   `case_class` values absent from the manifest; `quality` absent; an
   `identity.pinned` field differing between sides; or a case whose pairs
   straddle a gate (D10, D11, D19, D20, D21, D42, D43).
...
| D42 | The same cited `(record_id, run_id)` twice on one side is `TRIALS_DUPLICATE`, an evidence fault; the same run cited on *both* sides of one case stays legal | A median over copies of one measurement buys the approval D36's exact cardinality exists to deny; a run cited on both sides yields delta 0 and buys nothing | Counting citations rather than measurements — three copies of one trial then satisfy the three-trial floor |
| D43 | One measurement cited by two different cases is `TRIALS_REUSED`, an evidence fault naming the first case that claimed it | Per-side distinctness (D42) and `CASE_CLASS_MISSING` are both satisfied by four core cases carrying identical citations, so a single trial set would otherwise decide the whole gate — the cheapest approval to buy, and a run belongs to one workload, so citing it under two case classes is a category error | Checking distinctness per side only — closes the visible copy and leaves the cheaper cross-case one open |
| D44 | `build_record`'s `fleet` block folds only the strata that measured runs; a stratum that scanned to nothing contributes no key rather than a zero | D6: an absent measurement is not a zero, and a fleet total that silently absorbs an empty stratum reads as a measured aggregate over both | Summing both strata unconditionally — an empty Codex window would report a fleet total indistinguishable from a measured one |
| D45 | `quality` is allowed-but-not-required in the stratum's *structural* schema, so an absent block reaches `QUALITY_MISSING` (exit 3); a *present* block still owes all three fields at exit 2 | The verdict list names `quality` absent as an evidence fault, while D37 makes an absent `evaluator_stability` a document fault; requiring the block structurally raises `ManifestError` first and leaves `QUALITY_MISSING` unreachable dead code | Keeping `quality` in `STRATUM_REQUIRED_FIELDS` — the tool exits 2 where the design promises an `unmeasured` bundle, and a named diagnostic can never fire |
