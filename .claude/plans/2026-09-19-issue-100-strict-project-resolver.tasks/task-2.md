...
- `context-map-lint` requires the absolute repository root and absolute context-map path selected from `bindings.paths.context`; it validates that the map is inside the root and never resolves policy, reads legacy config, or searches default map locations (D8).
...
Change `scripts/context-map-lint.py` to require `--repo-root <absolute-root>` and `--context-map <absolute-map>`. Replace `find_map()` with argument validation: both paths must be absolute, the root must be a directory, and the context map must resolve beneath that root; a missing map remains a clean no-domain-doc result only when the retained snapshot supplied no context-map path and therefore the caller does not invoke the linter. The linter never calls `resolve-project`, reads `.agents/project.json`, reads legacy configuration, or searches `docs/CONTEXT-MAP.md`/`CONTEXT-MAP.md` itself.

Create `tests/test_context_map_lint.py` with subprocess tests that pass an explicit snapshot-derived map path and assert: a valid map exits 0; a malformed map returns findings; a relative/out-of-root `--context-map` exits 2; and the source contains none of the resolver/config/default-discovery tokens. Add `scripts/context-map-lint.py` to Task 4's tracked living-source scan and, in its installed descriptor, check `~/.agents/bin/context-map-lint` with the same forbidden-token assertion.
