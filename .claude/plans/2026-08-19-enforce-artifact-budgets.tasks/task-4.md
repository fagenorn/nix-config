...
**Interfaces:**
- Consumes: Task 1's `artifact-budget check`; D2, D5, D6, and D7.
- Produces: design report with existing `spec_path`, `adr_paths`, and `decisions` plus `state`, one design-spec `artifact`, and `notes`; grill report with existing decision/doc/open-item fields plus the same state/artifact shape; handoff report with `state`, one handoff `artifact`, and `notes`.
...
- Reports never inline contents, member lists, policy, logs, or decision-ledger rows; notes remain at most 500 characters.
...
def test_artifact_reports_are_bounded_root_only_shapes(self):
    for producer in (self.design, self.grill, self.handoff):
        ...
        self.assertIn("at most 500 characters", producer)
...
In `design`, put final measurement after its fresh-eyes fix pass. Define the exact generate/check/compact/recheck/complete-or-decompose sequence, the fixed report additions, and failed-measurement behavior. Make clear that a later grill or plan ledger append owns a new spec check.

In `grill-with-docs`, preserve glossary/ADR behavior, but make any spec/ledger mutation end in the same design-spec check. Its final report includes the spec root/metrics; if the grill did not change the spec, it must still obtain a current check rather than repeat an earlier claim.
