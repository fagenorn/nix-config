...
- Estimated implementation aggregate is 250–360 KiB. Estimated tracked process-artifact overhead already in `DELIVERY_BASE..DELIVERY_HEAD` is 110–125 KiB for the design and final plan package; generated review-package manifest/index/shard overhead is another 10–20 KiB. The estimated complete package is therefore 370–505 KiB across about 18 changed files, under the 524,288-byte aggregate cap. These are planning estimates, not permission to exceed either cap.
...
- Before dispatching each task, compare current cumulative per-file and aggregate review-package metrics with these caps. If an estimate was wrong, stop with `decompose_required` and add another sibling module/test file along the same schema/routing/scheduling boundary before implementation; splitting commits or test classes without splitting the oversized file is not remediation.
