...
- The authoritative design is `.claude/specs/2026-08-19-artifact-budget-policy-design.md`; implementations cite D1–D10 and do not duplicate decision rationale.
...
- All sizes are encoded bytes, not characters or model tokens; booleans are not integers for policy validation.
...
- A successful producer report carries one artifact root path, `budget_status: within_budget`, and exactly `root_bytes`, `total_bytes`, `file_count`, and `largest_member_bytes`; notes are at most 500 characters.
