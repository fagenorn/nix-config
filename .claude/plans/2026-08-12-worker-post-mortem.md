...
- **Superseded, retained for the record: every `grep -c … codex-plugin-cc.patch` gate below** (Task 1 Step 6, Task 2 Step 9, Task 3 Step 6). A zero-context patch carries no per-line file attribution, so a patch-wide count sums hunk lines from all touched files and cannot say which file a match sits in — which is exactly how the B1/B4/E2 corrections above came about. The authoritative gates are the `$STORE/...` counts in each task's Step 8 and Task 4's whole-issue block, which read real assembled source. Read a patch-file count as a weak smoke signal only, never as a per-file assertion; `CLAUDE.md` now states the rule.
...
- **Choice:** 13 tests in the new `tests/worker-postmortem.test.mjs` (3 in Task 1, 5 in Task 2, 4 in Task 3, 1 in Task 4), 1 test appended to `tests/commands.test.mjs` (Task 4), and 1 existing `runtime.test.mjs` test rewritten in place (count unchanged). Expected gates: after Task 1 `# tests 110 / # pass 106 / # fail 0 / # skipped 4`; after Task 2 `115 / 111 / 0 / 4`; after Task 3 `119 / 115 / 0 / 4`; after Task 4 `121 / 117 / 0 / 4`. Those four gates are what each task commit was held to and they all held. Two later fix waves added regression tests on top: the Phase-6 wave (`b9cc4b0`) 3, and the ship-time wave 2, so the branch head is `# tests 126 / # pass 122 / # fail 0 / # skipped 4` with 18 tests in `tests/worker-postmortem.test.mjs`. `# fail 0 / # skipped 4` is the invariant; the totals are a moving floor.
...
- **Alternative considered:** Passing `stateDir` into `reconcileWorkerLiveness` (a signature change to a function this plan deliberately leaves untouched, and issue #2's tests pin its behaviour); leaving it (rejected: it converts a rare-but-real condition into a permanently wrong record). Recorded here rather than papered over with a test that would prove nothing. **Amended after the final review:** this entry originally claimed no test could accompany it, because every test's seed workspace resolves to its own state dir. That was wrong — the divergence is reachable at the existing seams by seeding a record under one state dir while its `workspaceRoot` points elsewhere, and the fix wave's COR-02 test does exactly that. The gate is now covered.
...
### F1: the final review's two axes, and the four findings the fix wave closed

- **Question:** The whole-branch review ran two axes over `165a3b0..3b30bf9`. Conformance came back clean with two minors; the correctness axis (Codex, a real completed run — no fallback this time) raised three Important and one Minor. Which get fixed, which get parked, and does any of them mean backing up a phase?
- **Choice:** Four fixed in one wave (commit `b9cc4b0`), one parked with a ruling, one deferred.
  - **COR-01 — fixed.** `listJobs` capped reads at `MAX_JOBS` status-blind, so S1's active-record *storage* exemption was defeated on the *read* side: a wedged live record sitting behind 50 newer retained terminal records was on disk but invisible to `status`, to `status <id>` and to cancel. `listJobs` now takes the same optional `retain` predicate `pruneJobRecordsInStateDir` does, both consume one shared `selectRetainedJobs` helper, and a `listJobsRetainingActive` wrapper in `tracked-jobs.mjs` gives the policy a single home for all seven read call sites. `state.mjs` stays status-agnostic.
  - **COR-02 — fixed.** Both reviewer-runtime cleanup call sites in the SessionEnd hook still derived their path from `workspaceRoot`, so in exactly the divergence case S3 detects they aimed at an empty state dir while the real runtime dir leaked. A `cleanupReviewerRuntimeInStateDir` sibling in `runtime-home.mjs` — the same split idiom this plan established in `state.mjs` — fixes both; the "outside its owned root" guard stays on the shared path.
  - **COR-04 — fixed.** `resolveJobDeadline` ran *after* the spawn and threw `RangeError` for a finite positive timeout past the representable Date range, orphaning a live detached worker with no record. The deadline is now computed before the spawn and the function is total.
  - **CONF-M1 — fixed.** The spec still described the pre-S3 reconcile gate in three sentences (plus a fourth the fixer found). Corrected in place, same as S1's.
  - **COR-03 — parked.** The tightened preview filter is a format heuristic, not a provenance channel: output that began with a byte-exact bracketed ISO-with-millis prefix would still be admitted. Accurate, but the proposed correction — a separate channel for runtime progress — reverses the spec's central AC1 decision that the worker's fds land in the log the record already points at. That is a spec change, not a plan fix, so it is recorded for the human reviewer instead of scope-creeping the branch. Residual risk is negligible: `appendLogLine` is the only known producer of that prefix, and the codex child's stdio is piped, so it never reaches the worker's fd 2.
  - **CONF-M2 — deferred.** The duplicated three-line reviewer-cleanup block; the conformance axis independently triaged it `defer` (two lifecycle positions, not one policy).
- **Grounding:** Every finding was verified against the shipped tree before any edit — COR-01 at `state.mjs:190-194` plus the seven `listJobs` call sites, COR-02 at `runtime-home.mjs:76-80,94-102`, COR-03 at `job-control.mjs:57`, COR-04 at the call order in `enqueueBackgroundTask`, CONF-M1 at the spec's own lines. None was rejected as stale. COR-01 and COR-02 are both cases of a Phase-5 finding fixed on one side of a boundary and missed on the other, which is why they were worth fixing rather than deferring: each defeated a guarantee this branch itself promises. The conformance axis also triaged all twelve of the tasks' deferred and parked ledger lines as `defer`, so nothing else was outstanding at merge.
- **Alternative considered:** Deferring COR-01 as a pre-existing `listJobs` behaviour — rejected: the cap is pre-existing but this branch is what makes it bite, by making 50-record dirs the steady state, and it silently defeats AC3. Fixing COR-03 by adding a provenance channel — rejected as above. Backing up to Phase 2 for COR-03 — rejected: it changes no requirement or acceptance criterion, only a known limitation of an approved mechanism, so a spec loop would produce the same design.

### Resuming a fourth time, at Phase 7, with the branch complete
- **Question:** A fourth `--auto` run was reset, this time after the final review record (`bc8b810`) and before anything shipped. Phase 0's pre-flight says a single clean matching worktree is an orphan to remove. Remove and restart, or resume at Phase 7?
- **Choice:** Resume at Phase 7 and ship this branch as it stands. Phases 0–6 are complete and recorded above; nothing is re-executed or re-reviewed.
- **Grounding:** `git status --porcelain` is empty and the branch holds thirteen commits above its base, covering all four tasks (`9a3af56`, `cbc4376`, `090ba5b`, `7cbbe40`), the fix wave (`b9cc4b0`) and the final two-axis review record (`bc8b810`, entry F1). `gh pr list --state all --search "issue-10"` still returns no PR for this issue, so nothing shipped and no reviewer has seen it. The pre-flight's remove-the-orphan branch is written for scaffolding a run abandoned before it produced anything; applying it literally to a finished branch would discard ~3.5k committed lines and re-derive the same design, which inverts the rule's purpose.
- **Alternative considered:** Removing the worktree and re-running the flow from Phase 0 — rejected as above. Re-running the final review before shipping — rejected: it re-reviews an unchanged tree, and ship-issue runs its own PR review over the merge delta anyway, which is where a fresh pair of eyes actually adds signal.

### G1: the ship-time PR review, and the wave that closed it

- **Question:** ship-issue Phase 5 ran the full two-axis review over `c254fee..HEAD` on PR https://github.com/fagenorn/nix-config/pull/13 (the diff is far past the merge-delta thresholds, so the degraded path did not apply). Correctness (Codex) returned one Critical and two Important; conformance returned three Important, all stale prose, plus one Minor. Which get fixed before merge?
- **Choice:** All six applied in one wave, at `patchRevision = 6` — the same reasoning `b9cc4b0` used, since revision 6 has still never reached `main` under any content.
  - **COR-05 — fixed (Critical).** The write path defeated the read-path exemption exactly as `listJobs` had defeated the storage exemption. A config write is `loadState` → mutate → `saveState`, and `saveState` prunes the jobs dir against the list `loadState` produced; both halves were status-blind, so `/codex:setup --disable-review-gate` **deleted** a wedged live job's `.json` and `.log` once fifty newer terminal records became the steady state — the state this branch itself creates. `retain` now threads through `loadState`, `saveState`, `updateState`, `setConfig` and the `pruneJobRecords(cwd)` wrapper, and `setConfigRetainingActive` in `tracked-jobs.mjs` gives the write-path policy the single home `listJobsRetainingActive` gives the read path. `state.mjs` stays status-agnostic. Pinned by `a config write keeps a wedged active record and its log`.
  - **COR-06 — fixed (Important).** `terminalizeLiveSessionJob` swallowed a thrown `terminateProcessTree` and wrote `cancelled` with `pid: null` regardless. A refused kill (EPERM on a reparented worker) therefore claimed a clean cancellation for a worker that may still be running, and destroyed the one field a post-mortem reader needs. A thrown termination now records `failed` with the pid preserved and a message saying the worker may have outlived the session. `delivered: false` (ESRCH — the process was already gone) keeps writing `cancelled`, so the existing alive-worker and no-pid paths are untouched.
  - **COR-07 — fixed (Important).** `resolveJobDeadline` recorded any representable deadline, but the worker's budget is a `setTimeout` and Node clamps a delay above 2^31-1 ms to 1 ms. A `--timeout-ms` in the band above the timer maximum but inside the Date range (e.g. `3e9`) made the job die almost immediately while its record advertised a deadline five weeks out with `overdue: false`. The deadline is now capped at `MAX_ENFORCEABLE_TIMEOUT_MS`: an unenforceable budget reports no deadline, the same answer an unrepresentable one already got. Pinned by `a timeout the runtime cannot enforce records no deadline`.
  - **CONF-I1 / I2 / I3 and the Minor — fixed (docs).** The spec's two surviving "`listJobs` applies it unconditionally" sentences and its "passing no `retain`" prescription (both falsified by COR-01 and now COR-05), the test-count claims that stopped at Task 4's 121, and the patch-wide `grep -c` gates that the head commit's own `CLAUDE.md` rule forbids — the last annotated as superseded by the `$STORE` gates rather than deleted, since they are the record of what each task ran.
- **Grounding:** Every finding was re-verified against the live patched tree before editing. COR-05 traced through `state.mjs:167,227,244,248` and the sole production caller at `codex-companion.mjs:231`; the import direction (`tracked-jobs.mjs` → `state.mjs`) is what forces the predicate to come from the caller. COR-06 read `process.mjs:100-118`, where a non-`ESRCH` errno propagates as a throw. COR-07 was confirmed against the existing unrepresentable-deadline test, whose own comment already relies on the clamped `setTimeout` — which is why the fix had to land on the reporting side and not on enforcement. Gates after the wave: `# tests 126 / # pass 122 / # fail 0 / # skipped 4` env-scrubbed, patch re-applies `--unidiff-zero` at the pin and regenerates byte-identically, `just build` green.
- **Alternative considered:** For COR-05, defaulting the cwd-scoped prune to retain-active inside `state.mjs` — rejected, it would make that module status-aware and invert the import direction. For COR-06, leaving the record active so heal-on-read eventually flips it — rejected, AC2 exists so a session's records do not stay active forever. For COR-07, rejecting an over-large `--timeout-ms` at the option boundary or clamping enforcement up to the timer maximum — both rejected: each breaks the fix wave's own `a timeout whose deadline is unrepresentable still lands a complete job record`, which asserts the enqueue succeeds and the clamped timeout resolves the job promptly. Neither residual from F1 (COR-03, CONF-M2) was re-opened; the ship reviewer independently checked both rulings against the live code and confirmed them.
- **Not fixed, recorded:** COR-06 has no test. Forcing a refused kill needs an unkillable-but-live pid, and the hook calls `terminateProcessTree` with no injection seam; the only cheap candidate, pid 1, would make the process-group branch call `kill(-1, SIGTERM)` — "signal every process the caller may signal" — which is unacceptable in a test suite. The delivered path stays pinned by `session end terminates a live worker and records it cancelled`.
...
Run (from `$SCRATCH`): `env -u CLAUDE_PLUGIN_DATA -u CODEX_COMPANION_SESSION_ID -u CODEX_COMPANION_TRANSCRIPT_PATH node --test tests/*.test.mjs`
Expected: `# tests 121 / # pass 117 / # fail 0 / # skipped 4` (this task's own gate; later fix waves raise the total to 126 — see the test-budget entry above). In particular the existing table assertion in `status shows phases, hints, and the latest finished job` must still pass — it matches the `Elapsed` cell with `.*`, and that job has no `deadlineAt`, so the cell is a bare duration.
...
- **R8 / AC4** (suite green env-scrubbed; `patchRevision` 5→6 with `just build` green): every task's suite gate (110 → 115 → 119 → 121 tests, `# fail 0`, `# skipped 4` throughout), Task 1 Steps 7-8 (bump + build), Task 4 Steps 7-8 (determinism, `patchRevision = 6`, `.p6` closure content checks). At the branch head, after both fix waves, the same env-scrubbed command reports `# tests 126 / # pass 122 / # fail 0 / # skipped 4` with `just build` green and `patchRevision = 6` — both waves ride revision 6, since it never reached `main` under any earlier content.
