...
- `Verdict and scope`: the exact line ``- Verdict: `pending` ``; run, attempt, owner, exact worktree, inclusive logical ceiling, reviewed HEAD; one-trace limitation; a closed list of terminal-ledger state, representative merge, both controller terminal-prefix anchors and maximum records, final matrix replay, and final evidence commit as remaining.
...
- `Controller input`: visibly pending rows for both controllers; columns `role`, `session`, `prefix path`, `bytes`, `sha256`, `selected at`, `logical`, `cached`, `fresh`, `<=150000`, and `status`. Do not place a provisional maximum in either row before its terminal prefix is closed.
