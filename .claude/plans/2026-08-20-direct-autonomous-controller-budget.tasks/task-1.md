...
**Interfaces:**
- Consumes: the committed design and full plan package at the fresh delegated owner's exact reviewed HEAD; the `direct-75-000002` owner envelope; the pre-rollover controller session and exact structured Phase-5 `delegate` observation; the fresh owner's session metadata and adoption record; canonical ledger path; issue-74 merge `f3fac9554761d0c3085d70bf4526cf3e7486de3e` and base `c780b38f613c59a7d6674dc081d9f67666054ebf`; activated installed files; historical sessions `01a01acf-a82d-7953-813b-401d252e02da` and `01a01bd9-8c37-7181-86de-58c82f6a643a`; D1–D9.
...
- The report records the immutable issue-74 merge/base, reviewed HEAD, activated store path, deployment/session timestamps, installed-file identities, the exact rollout prefix through the Phase-5 `delegate` observation, fresh-owner identity, pre-rollover Git range, structured dispatch inventory, and rechecked historical observations only when directly measured.
...
- The pending report does not name a representative-run merge, terminal outcome, terminal-ledger digest, either controller's terminal prefix size/digest or maximum token record, final verdict, or final evidence commit as if known. It names each as remaining evidence.
...
assert re.findall(r"(?m)^- Verdict: `(pending|certified|not certified|unknown)`$", text) == ["pending"]
...
3. Extract only `session_meta` identity/start/cwd plus the structured owner/adoption/progress/dispatch/task records needed by the report. Derive the pre-rollover observation-prefix endpoint from the exact persisted Phase-5 `delegate` result, then record `wc -c` over that exact prefix and `shasum -a 256` over exactly those bytes. Record the compact Phase-5 fields `run_id`, `issue`, `attempt`, `owner`, `worktree`, `phase`, and `phase_action` from that sealed observation. Do not label this observation prefix as the controller's terminal prefix or derive a controller maximum from it.
4. Enumerate every session associated with this run from structured dispatch/task ownership records, not from a free-text hit. For each, record session ID, role, authority evidence, and D2 inclusion decision. Require the fresh owner's session ID to differ from the pre-rollover controller and its start to follow the Phase-5 observation. Compare its adoption envelope with the run/attempt/owner/worktree values only if that payload is observable; otherwise record the exact comparison as unavailable/pending and retain the assigned fresh controller for terminal measurement rather than inferring equality or failure.
...
If a required pre-terminal fact is unavailable, record that fact as unavailable with `remaining`; do not infer it. Use `fail` only for an observed contradiction or violated invariant. If deployment ordering actually fails, preserve that observed failure while keeping the overall pre-terminal verdict `pending`. Additional D2-required controllers add measurement obligations and are not themselves failures.
...
- `Verdict and scope`: the exact line ``- Verdict: `pending` ``; run, attempt, owner, exact worktree, inclusive logical ceiling, reviewed HEAD; one-trace limitation; a closed list of terminal-ledger state, representative merge, every D2-required controller terminal-prefix anchor and maximum record, final matrix replay, and final evidence commit as remaining.
...
- `Lifecycle trace`: direct-owner and fresh-owner session identities, sealed Phase-5 observation-prefix path/bytes/digest, exact Phase-5 `delegate` observation, unchanged envelope comparison, and current role-inventory table.
- `Controller input`: visibly pending rows for every D2-required controller plus any assigned controller retained because adoption evidence is unavailable; columns `role`, `session`, `prefix path`, `bytes`, `sha256`, `selected at`, `logical`, `cached`, `fresh`, `<=150000`, and `status`. Do not place a provisional maximum in any row before its terminal prefix is closed.
